...
* High assurance in integrity of DID Documents and DID Document history, based on delegated Proof of Stake (dPoS) consensus algorithm
...
* Blockchain-based validation of cryptographic keys stored within the `authentication` section of DID Documents, ensuring that only the controller of those keys has the ability to update or change the DID Document
...
* Clear and consistent pricing for DIDs and DID-Linked Resources
* Decentralized governance framework for updates to software updates or pricing changes for DID Method on cheqd Network, voted through by the validator nodes and delegators who stake the $CHEQ token
...
| Muiltiple implementation routes, via SDKs and APIs as well as full support in the Universal Registrar and Resolver. Clear documentation and tutorials written for each seperate integration pathway  |
| **Community adoption and support** |
| Over 300,000 DIDs on testnet and support in four of the largest industry SDKs, as well as over ten identity applications |
...
| Yes, supports BBS privacy-preserving cryptographic key formats embedded within the DIDDoc. Any DID-Core compatible key formats can be defined in verification method relationships within the DID Documents, even if they are not natively validated against by the underlying cheqd network. |
...
| Yes, full suite of DID traits are identified and referenced. |
