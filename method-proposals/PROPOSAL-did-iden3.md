...
| **Scalability and performance** | Highly scalable; `did:iden3` has been tested by leading banks in production environments. |
...
| **Compliance with relevant regulations and best practices** | Yes, zk-proofs allow achieving GDPR’s data minimization objective. The DID method is registered with the W3C and compliant with the W3C DID Specification. |
...
| **Governance: Clear frameworks for updates, dispute resolution, and decision-making** | Yes. Governed by the nonprofit "0KIMS Association." |
...
| **Sustainability: Energy efficiency and eco-friendly infrastructure** | Identifier management is inexpensive. The blockchain infrastructure deployed typically uses a proof-of-stake consensus mechanism, which is environmentally friendly. |
| **Economic Feasibility: DIDs costs of use must be reasonable** | It is free to create `did:iden3` identifiers. Some operations like key rotation and claims revocation incur minimal blockchain gas costs (cents of the dollar). |
| **Legal Recognition: Cross-border frameworks for DID acceptance** | No. |
| **Revocation and Recovery: Decentralized mechanisms for key rotation and DID recovery** | Yes, fully supported. |
...
| **Reliable and predictable-latency operation, for updating and resolving** | Generally yes, depends on the blockchain used. |
| **Resolution should not require additional state or context** | Basic resolution is done from the identifier itself and the state contract of the blockchain. |
| **DIDs are permanent and immutable account identifiers** | Yes, the identifier remains stable, while key states can change via [on-chain identity state transitions](https://docs.iden3.io/getting-started/state-transition/state-transition/). |
| **Consider support for various DID Traits: <https://identity.foundation/did-traits/>** | Yes. Update supported, Service Endpoints can be updated, Deactivate supported, Transactional Fees, Verification Methods can be updated, Globally Resolvable, DID Document History, Cryptographically signed DID Document History, Decentrally Hosted, Privacy Preserving Crypto - niZKPs. |
| **Consider categories defined by DID Rubric: <https://www.w3.org/TR/did-rubric/>** | Not evaluated. |
| **Who WANTS to standardize the DID method and commits to doing the work?** | Yes, Privado ID and other private entities are committed to standardization. |
